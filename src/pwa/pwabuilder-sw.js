...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(precacheFiles).then(() => {
...
function cacheFirstFetch(event) {
  event.respondWith(fromCache(event.request).then(
    (response) => {
      ...
      event.waitUntil(fetch(event.request).then(response => updateCache(event.request, response)));
